...
extern int gbl_verbose_decode_errors;

int gbl_request_dump_size = 256;

void dump_buf_limited(const uint8_t *p_buf, const uint8_t *p_buf_end) {
    if (p_buf == NULL || p_buf_end == NULL)
        return;
    if (p_buf_end < p_buf)
        return;
    int limit = p_buf_end - p_buf;
    if (limit > gbl_request_dump_size)
        limit = gbl_request_dump_size;
    fsnapf(stderr, p_buf, limit);
}

#define SAVE_BUFFER_START(x) \
    const uint8_t *_p_buf_start = x; \
    const uint8_t *_p_buf_last = _p_buf_start;

#define SAVE_BUFFER_PROGRESS(p_buf) \
    if (p_buf != NULL) \
        _p_buf_last = p_buf;

#define DUMP_ON_ERROR(p_buf, p_buf_end, expected_size) \
    do {         \
        if (unlikely((p_buf == NULL) || (p_buf_end - _p_buf_start) < expected_size) && gbl_verbose_decode_errors) { \
            fprintf(stderr, "%s: %p to %p bufsz %"PRIu64"\n", __func__, p_buf, p_buf_end, (uint64_t)(p_buf_end - _p_buf_start)); \
            fprintf(stderr, "%s: last decoded at offset %"PRIu64"\n", __func__, (uint64_t)(_p_buf_last - _p_buf_start)); \
            dump_buf_limited(_p_buf_start, p_buf_end); \
        } \
    } while(0)
                       ...
                       const uint8_t *p_buf_end)
{
    ...
    DUMP_ON_ERROR(p_buf, p_buf_end, 0);
                       ...
                       const uint8_t *p_buf_end, int comdbg_flags)
{
    ...
    if (p_buf_end < p_buf || BLOCK_FWD_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, BLOCK_FWD_LEN);
        return NULL;
    }
    PUTFUNC
                          ...
                          uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || BLOCK_NESTED_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, BLOCK_NESTED_LEN);
        return NULL;
    }
                       ...
                       const uint8_t *p_buf_end, int comdbg_flags)
{
    ...
    if (p_buf_end < p_buf || BLOCK_RSP_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, BLOCK_RSP_LEN);
        return NULL;
    }
    PUTFUNC
                         ...
                         uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || BLOCK_RSPKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, BLOCK_RSPKL_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || BLOCK_RSPKL_POS_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, BLOCK_RSPKL_POS_LEN);
        return NULL;
    }
                       ...
                       const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || BLOCK_ERR_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, BLOCK_ERR_LEN);
        return NULL;
    }
    ...
    const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || LONGBLOCK_REQ_PRE_HDR_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, LONGBLOCK_REQ_PRE_HDR_LEN);
        return NULL;
    }
    ...
    const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || LONGBLOCK_FWD_PRE_HDR_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, LONGBLOCK_FWD_PRE_HDR_LEN);
        return NULL;
    }
                      ...
                      uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || LONGBLOCK_REQ_HDR_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, LONGBLOCK_REQ_HDR_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || LONGBLOCK_RSP_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, LONGBLOCK_RSP_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_HDR_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_HDR_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_ADD_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_ADD_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_ADDSEC_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_ADDSEC_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_SEQ_OLD_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_SEQ_OLD_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_DEL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_DEL_LEN);
        return NULL;
    }
                            ...
                            uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_SEQ2_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_SEQ2_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_DELSEC_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_DELSEC_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_UPVRRN_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_UPVRRN_LEN);
        return NULL;
    }
                       ...
                       uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_UPDRRNKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_UPDRRNKL_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_USE_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_USE_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_USEKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_USEKL_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_TZSET_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_TZSET_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_SCSMSK_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_SCSMSK_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_ADDDTA_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_ADDDTA_LEN);
        return NULL;
    }
                       ...
                       uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_UPDBYKEY_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_UPDBYKEY_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_ADDKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_ADDKL_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_ADDKEY_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_ADDKEY_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_KEYOP_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_KEYOP_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_DELETE_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_DELETE_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_DELKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_DELKL_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_DELKEY_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_DELKEY_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_UPDRRN_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_UPDRRN_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_QBLOB_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_QBLOB_LEN);
        return NULL;
    }
                       ...
                       uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_RNGDELKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_RNGDELKL_LEN);
        return NULL;
    }
                       ...
                       uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_SETFLAGS_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_SETFLAGS_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_CUSTOM_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_CUSTOM_LEN);
        return NULL;
    }
                            ...
                            uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_QADD_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_QADD_LEN);
        return NULL;
    }
                            ...
                            uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_TRAN_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_TRAN_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_SQL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_SQL_LEN);
        return NULL;
    }
    ...
    const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_SET_MODNUM_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_SET_MODNUM_LEN);
        return NULL;
    }
                       ...
                       uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_DELOLDER_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_DELOLDER_LEN);
        return NULL;
    }
    ...
    const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_SQL_PARMS_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_SQL_PARMS_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || FSTBLK_HEADER_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, FSTBLK_HEADER_LEN);
        return NULL;
    }
                          ...
                          uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || FSTBLK_RSPOK_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, FSTBLK_RSPOK_LEN);
        return NULL;
    }
                           ...
                           uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || FSTBLK_RSPERR_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, FSTBLK_RSPERR_LEN);
        return NULL;
    }
                              ...
                              uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || FSTBLK_PRE_RSPKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, FSTBLK_PRE_RSPKL_LEN);
        return NULL;
    }
                          ...
                          uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || FSTBLK_RSPKL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, FSTBLK_RSPKL_LEN);
        return NULL;
    }
                                    ...
                                    const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || CLIENT_BLOB_TYPE_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, CLIENT_BLOB_TYPE_LEN);
        return NULL;
    }
                              ...
                              const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || OPREC_SIZE > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, OPREC_SIZE);
        return NULL;
    }
                                  ...
                                  const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || COMMITREC_SIZE > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, COMMITREC_SIZE);
        return NULL;
    }
                            ...
                            uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_DBGLOG_COOKIE_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_DBGLOG_COOKIE_LEN);
        return NULL;
    }
                             ...
                             uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    if (p_buf_end < p_buf || PACKEDREQ_UPTBL_LEN > (p_buf_end - p_buf)) {
        DUMP_ON_ERROR(p_buf, p_buf_end, PACKEDREQ_UPTBL_LEN);
        return NULL;
    }
